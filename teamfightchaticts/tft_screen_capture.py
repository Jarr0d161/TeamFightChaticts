...
class TFTTesseractScreenCapture:
    ...
    def capture_level(self) -> Tuple[int, int]:
        screenshot = self._capture_screenshot(
            box=(400, 880, 450, 910), crop=(5, 10, 40, 25)
        )
    ...
    def capture_item_locations(
        ...
        return [
            (match[0], match[1])
            for template_file in ICON_FILES
    ...
    def _scan_numeric_text_ocr(self, image: np.ndarray) -> str:
        ...
        return pytesseract.image_to_string(
            edge_filtered_image, config=tess_params
        ).strip()

    def _adaptive_threshold(self, image):
        return cv2.adaptiveThreshold(
            image, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 61, 11
        )
